...
public class HangmanGame extends Game {
				...
				if (amountBet <= 0) {
					...
					while (lives >= 1) {
						...
						if (this.phrase.indexOf(guess) == -1) {
							...
							for (int index = this.phrase.indexOf(guess); // set index to the first occurrance
									index >= 0; // as long as it's not -1 there is another of the same character
									index = this.phrase.indexOf(guess, index + 1) // set the index to the next index of
																					// the char
									) {
	...
	private String getPhrase() {
		...
		String[] phrases = { "Test Phrase", "Sunny sky in morning shepards warning",
				"Sunny sky at night shepards delight", "A dime a dozen", "Chip on the shoulder", "A piece of cake",
				"An arm and a leg", "Back to sqaure one", "Barking mad", "With a grain of salt" };
